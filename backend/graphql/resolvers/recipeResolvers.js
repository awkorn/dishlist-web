...
const recipeResolvers = {
  Query: {
    ...
    getRecipe: async (_, { id, userId }) => {
      ...
      // If recipe is in at least one public dishlist, anyone can view it
      const publicDishLists = await DishList.find({
        _id: { $in: recipe.dishLists },
        visibility: "public"
      });
      
      if (publicDishLists.length > 0) {
        return recipe;
      }
      
      // Check if user has access to any of the dishlists containing the recipe
    ...
    getDishListRecipes: async (_, { dishListId, userId }) => {
      ...
      const canAccess = 
        ...
        (dishList.visibility === "shared" && dishList.sharedWith.includes(userId)); // Shared with user
        
      if (!canAccess) {
        throw new Error("You don't have permission to view recipes in this dishlist");
      }
  ...
  Mutation: {
    ...
    addRecipeToDishList: async (_, { recipeId, dishListId, userId }) => {
      ...
      if (!canAdd) {
        throw new Error("You don't have permission to add recipes to this dishlist");
      }
    ...
    removeRecipeFromDishList: async (_, { recipeId, dishListId, userId }) => {
      ...
      if (dishList.userId !== userId && !dishList.collaborators.includes(userId)) {
        throw new Error("You don't have permission to remove recipes from this dishlist");
      }
